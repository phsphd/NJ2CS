...
namespace SchwabApiCS
{
    public partial class SchwabApi
    {
        ...
        public long? OrderExecuteHash(string accountHash, Order order)
        {
            return WaitForCompletion(OrderExecuteNewAsync(accountHash, order));
        }
        /// <summary>
        ...
        public async Task<ApiResponseWrapper<long?>> OrderExecuteNewAsync(string accountNumber, Order order)
        {
       	...
       	public async Task<ApiResponseWrapper<long?>> OrderExecuteHashAsync(string accountHash, Order order)
        {
            if (order.orderLegCollection != null)
            {
                foreach (var o in order.orderLegCollection)
                {
                    if ((o.instruction == "BUY" || o.instruction == "SELL") && o.instrument.assetType == "OPTION")
                        throw new SchwabApiException("OrderExecuteHashAsync error: Instruction " + o.instruction + " invalid with Options.");
                }
            }
			...
			string orderurl=OrdersBaseUrl + "/accounts/" + accountHash + "/orders";
			UpdateStatus($"OrderExecuteHashAsync: {orderurl},  {jsonOrder}");
            var result = await Post<long?>(orderurl, jsonOrder);
            if (result.ResponseMessage != null && result.ResponseMessage.IsSuccessStatusCode)
            {
                var pathParts = result.ResponseMessage.Headers.Location.LocalPath.Split('/');
                result.Data = Convert.ToInt64(pathParts[pathParts.Length-1]); // get last part, should be SchwabOrderID
				UpdateStatus($"OrderExecuteHashAsync OrderID  {result.Data} ");
            } 
            else
            {
                result.HasError = true;
